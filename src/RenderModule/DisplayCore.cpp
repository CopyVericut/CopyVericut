...
TubeNext::DisplayCore::DisplayCore(OccView*parent)
{
...
void TubeNext::DisplayCore::DisplayShape(TopoDS_Shape shape, Quantity_Color color, double transparency=0, Graphic3d_MaterialAspect material, Standard_Boolean theToUpdateViewer)
{
...
void TubeNext::DisplayCore::DisplayShape(TopoDS_Shape shape, double transparency, Graphic3d_MaterialAspect material, Standard_Boolean theToUpdateViewer)
{
	Handle(AIS_Shape) ais_shape = new AIS_Shape(shape);
	ais_shape->SetMaterial(Graphic3d_MaterialAspect(Graphic3d_NOM_STEEL));
	// Set Display mode
...
void TubeNext::DisplayCore::DisplayShape(TopoDS_Shape shape, Quantity_Color color, Aspect_TypeOfLine theType, Standard_Boolean theToUpdateViewer)
/*
...
void TubeNext::DisplayCore::Read_step_file_with_names_colors1(string filename)
{
...
int TubeNext::DisplayCore::Read_step_file(string filepath,string filename)
{
...
int TubeNext::DisplayCore::Displaytriehedron()
{
...
int TubeNext::DisplayCore::DisplaytWCSriehedron(const gp_Pnt& P, const gp_Dir& Vx, const gp_Dir& Vy)
{
...
int TubeNext::DisplayCore::DisplayPlane()
{	
...
int TubeNext::DisplayCore::DisplayCube()
{	
...
int TubeNext::DisplayCore::DisplayAxis(Vector_Type thevector,gp_Pnt point, gp_Dir diretion, Quantity_Color color, Standard_Real length, Standard_Real width)
{
	...
	switch (thevector)
	{
	case TubeNext::X_Vector:
		ShapeManeger["X方向"] = new shape(axis);
		break;
	case TubeNext::Y_Vector:
		ShapeManeger["Y方向"] = new shape(axis);
		break;
	case TubeNext::Z_Vector:
...
void TubeNext::DisplayCore::HidePlane()
{
...
void TubeNext::DisplayCore::RemoveAxis(Vector_Type thevector)
{
	switch (thevector)
	{
	case TubeNext::X_Vector:
...
TubeNext::shape::shape(Handle(AIS_Shape) aisShape)
{
	AisShape = aisShape;
}

TubeNext::shape::shape(Handle(AIS_Trihedron) aisrihedron)
{
	Aisrihedron = aisrihedron;
}

TubeNext::shape::shape(Handle(AIS_Plane) aisplane)
{
	AisPlane = aisplane;
}

TubeNext::shape::shape(Handle(AIS_ViewCube) aisviewcube)
{

	AISViewCube = aisviewcube;
}

TubeNext::shape::shape(Handle(AIS_Axis) aisvaxis)
{
	AISAxis = aisvaxis;
}


void* TubeNext::shape::Value()
{
	return NULL;
}
